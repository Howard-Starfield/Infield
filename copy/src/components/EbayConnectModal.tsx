  ...
  return (
    <div style={{
      ...
      <motion.div 
        ...
        style={{
          width: '100%', maxWidth: '500px', background: 'var(--surface-container-lowest)',
      ...
      >
        ...
        <div style={{ padding: '24px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 40, height: 40, borderRadius: '10px', background: 'var(--primary-container)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Globe size={20} color="var(--on-primary-container)" />
            </div>
            <div>
              <h2 style={{ fontSize: '18px', fontWeight: 700, margin: 0 }}>Connect eBay Account</h2>
              <p style={{ fontSize: '12px', color: 'var(--on-surface-variant)', margin: 0 }}>Secure OAuth Integration</p>
            </div>
          </div>
          ...
          <AnimatePresence mode="wait">
            {step === 'config' && (
              <motion.div key="config" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                ...
                <p style={{ fontSize: '13px', color: 'var(--on-surface-variant)', marginBottom: '24px', lineHeight: 1.6 }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: '12px' }}>
                      <button 
                        ...
                        style={{ ...toggleStyle, background: transport === 'loopback' ? 'rgba(128,131,255,0.1)' : 'var(--surface-container-low)', border: `1px solid ${transport === 'loopback' ? 'var(--primary)' : 'rgba(255,255,255,0.05)'}` }}
                      >Local Listener</button>
                      <button 
                        onClick={() => setTransport('https_bridge')}
                        style={{ ...toggleStyle, background: transport === 'https_bridge' ? 'rgba(128,131,255,0.1)' : 'var(--surface-container-low)', border: `1px solid ${transport === 'https_bridge' ? 'var(--primary)' : 'rgba(255,255,255,0.05)'}` }}
                      >HTTPS Bridge</button>
                    </div>
                  </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                      {[
                        ...
                        <div key={s.id} 
                          ...
                          style={{ 
                            padding: '10px', borderRadius: '10px', background: scopes[s.id as keyof typeof scopes] ? 'rgba(128,131,255,0.1)' : 'var(--surface-container-low)',
                            border: `1px solid ${scopes[s.id as keyof typeof scopes] ? 'var(--primary)' : 'rgba(255,255,255,0.05)'}`,
                            cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', transition: 'all 0.2s'
                          }}
                        >
                          <div style={{ width: '16px', height: '16px', borderRadius: '4px', border: '1px solid var(--primary)', background: scopes[s.id as keyof typeof scopes] ? 'var(--primary)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            {scopes[s.id as keyof typeof scopes] && <CheckCircle2 size={12} color="#fff" />}
                          </div>
                          <span style={{ fontSize: '12px', color: scopes[s.id as keyof typeof scopes] ? 'var(--on-surface)' : 'var(--on-surface-variant)' }}>{s.label}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {step === 'start' && (
              <motion.div key="start" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '12px', color: 'rgba(255,255,255,0.4)', marginBottom: '8px' }}>Account Label (Optional)</label>
                ...
                <button 
                  ...
                  style={{ ...btnStyle, marginTop: '32px' }}
                >
                  {loading ? <Loader2 className="animate-spin" /> : 'Launch Browser'}
                </button>
              </motion.div>
            )}

            {step === 'waiting' && (
              <motion.div key="waiting" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} style={{ textAlign: 'center' }}>
                <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: 'rgba(128,131,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px auto' }}>
                  <Loader2 size={32} color="var(--primary)" className="animate-spin" />
                </div>
                <h3 style={{ fontSize: '18px', fontWeight: 600, marginBottom: '8px' }}>Waiting for eBay...</h3>
                ...
                <button 
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '0 auto', background: 'none', border: 'none', color: 'var(--primary)', cursor: 'pointer', fontWeight: 600 }}
                >
