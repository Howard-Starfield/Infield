...
const SYSTEM_AUDIO_TUNING_KEYS = new Set<keyof Settings>([
  "system_audio_max_chunk_secs",
  "system_audio_paragraph_silence_secs",
  "system_audio_vad_hangover_secs",
]);

function isSystemAudioTuningKey(key: keyof Settings): boolean {
  return SYSTEM_AUDIO_TUNING_KEYS.has(key);
}

/** Debounce Rust persists while dragging so the UI stays responsive and sliders are not disabled. */
const systemAudioPersistTimers = new Map<
  string,
  ReturnType<typeof setTimeout>
>();

function clearSystemAudioPersistTimers() {
  for (const t of systemAudioPersistTimers.values()) {
    clearTimeout(t);
  }
  systemAudioPersistTimers.clear();
}

function scheduleSystemAudioTuningPersist<K extends keyof Settings>(
  get: () => SettingsStore,
  key: K,
  run: (value: Settings[K]) => Promise<unknown>,
) {
  const sk = String(key);
  const prev = systemAudioPersistTimers.get(sk);
  if (prev) clearTimeout(prev);
  const t = setTimeout(() => {
    systemAudioPersistTimers.delete(sk);
    ...
    void (async () => {
      try {
        ...
        console.error(`Failed to persist ${sk}:`, error);
      }
    })();
  }, 140);
  systemAudioPersistTimers.set(sk, t);
}
...
const settingUpdaters: {
  ...
  show_onboarding: (value) =>
    commands.changeShowOnboardingSetting(value as boolean),
  ...
  auto_create_note: (value) =>
    commands.changeAutoCreateNoteSetting(value as boolean),
  auto_tag_enabled: (value) =>
    commands.changeAutoTagEnabledSetting(value as boolean),
  llm_model_path: (value) =>
    commands.changeLlmModelPathSetting(value as string | null),
  llm_gpu_enabled: (value) =>
    commands.changeLlmGpuEnabledSetting(value as boolean),
  ...
  system_audio_max_chunk_secs: (value) =>
    commands.changeSystemAudioMaxChunkSecsSetting(value as number),
  system_audio_paragraph_silence_secs: (value) =>
    commands.changeSystemAudioParagraphSilenceSecsSetting(value as number),
  system_audio_vad_hangover_secs: (value) =>
    commands.changeSystemAudioVadHangoverSecsSetting(value as number),
};
...
export const useSettingsStore = create<SettingsStore>()(
  subscribeWithSelector((set, get) => ({
      ...
      try {
        ...
        if (result.status === "ok") {
          ...
          const normalizedSettings: Settings = {
            ...settings,
            system_audio_paragraph_silence_secs:
              settings.system_audio_paragraph_silence_secs ?? 1.5,
            system_audio_vad_hangover_secs:
              settings.system_audio_vad_hangover_secs ?? 0.51,
    ...
    updateSetting: async <K extends keyof Settings>(
      ...
      const audioTuning = isSystemAudioTuningKey(key);
      ...
      try {
        ...
        if (updater) {
          if (audioTuning) {
            scheduleSystemAudioTuningPersist(get, key, updater);
          } else {
            await updater(value);
          }
