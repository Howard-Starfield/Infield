        ...
        let worker = std::thread::spawn(move || {
            ...
            match init_result {
                Ok((stream, sample_rate)) => {
                    ...
                    // Pause the stream before dropping it. On Windows (WASAPI), the
                    // audio callback runs on a system-managed thread that can fire one
                    // more time after the stream object is dropped, accessing freed
                    // closure memory and causing STATUS_ACCESS_VIOLATION. Calling
                    // pause() stops WASAPI callbacks synchronously so that drop() is safe.
                    let _ = stream.pause();
                    drop(stream);
                }
                Err(error_message) => {
...
mod tests {
    ...
    #[test]
    fn run_consumer_processes_shutdown_without_audio_chunks() {
        let (_sample_tx, sample_rx) = mpsc::channel::<AudioChunk>();
        let (cmd_tx, cmd_rx) = mpsc::channel::<Cmd>();
        ...
        std::thread::spawn(move || {
            run_consumer(16_000, None, sample_rx, cmd_rx, None, stop_flag_for_thread);
            let _ = done_tx.send(());
        });
        ...
        assert!(
            ...
            "run_consumer should process shutdown without waiting for new audio"
        );
    }
}
