...
/// Webview logical size — keep in sync with [Handy-main/src/overlay/RecordingOverlay.css]
/// (max `.recording-overlay` footprint; pill uses `width: max-content` + glow bleed).
const OVERLAY_PILL_WIDTH_PX: f64 = 236.0;
const OVERLAY_PILL_HEIGHT_PX: f64 = 38.0;
const OVERLAY_GLOW_BLEED_PX: f64 = 14.0;
const OVERLAY_WIDTH: f64 = OVERLAY_PILL_WIDTH_PX + OVERLAY_GLOW_BLEED_PX * 2.0;
const OVERLAY_HEIGHT: f64 = OVERLAY_PILL_HEIGHT_PX + OVERLAY_GLOW_BLEED_PX * 2.0;
...
pub fn create_recording_overlay(app_handle: &AppHandle) {
    ...
    // Position comes from settings.overlay_position + monitor geometry (not window-state:
    // recording_overlay is denylisted so it is not saved/restored). We apply it right after
    // create; update_overlay_position() also runs when the user changes the setting.
...
/// Shows the loopback recording overlay (system audio capture)
pub fn show_loopback_recording_overlay(app_handle: &AppHandle) {
    show_overlay_state(app_handle, "loopback_recording");
}
...
pub fn hide_recording_overlay(app_handle: &AppHandle) {
    ...
    if let Some(overlay_window) = app_handle.get_webview_window("recording_overlay") {
        ...
        // IMPORTANT: window.hide() must be dispatched to the main/UI thread on
        // Windows. Calling it directly from a background thread causes a
        // STATUS_ACCESS_VIOLATION because WebView2/Win32 window operations
        // require being on the thread that owns the message pump.
        // We sleep on the background thread for the animation delay, then
        // re-dispatch hide() back to the main thread via run_on_main_thread.
        let window_clone = overlay_window.clone();
...
pub fn emit_loopback_levels(app_handle: &AppHandle, levels: &[f32]) {
    let _ = app_handle.emit("loopback-level", levels);
    if let Some(overlay_window) = app_handle.get_webview_window("recording_overlay") {
        let _ = overlay_window.emit("loopback-level", levels);
    }
}
