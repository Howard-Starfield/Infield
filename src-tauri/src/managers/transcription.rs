...
/// yields so the two don't starve each other for CPU / GPU.
pub(crate) fn transcription_session_holds_model(app: &AppHandle) -> bool {
    if app
...
/// RAII: decrements [`TranscriptionManager::active_leases`] on drop (long-audio import keep-alive, spec §3.6).
...
impl TranscriptionManager {
        ...
        // Start the idle watcher
        {
            ...
            let handle = thread::spawn(move || {
                ...
                while !shutdown_signal.load(Ordering::Relaxed) {
                    ...
                    // While recording (mic or system audio), keep the idle timer fresh so the
                    // model is never unloaded mid-session.
                    if transcription_session_holds_model(&app_handle_cloned) {
    ...
    /// Reset the idle timer to now. Public so the import pipeline can keep it fresh between segments.
    pub fn touch_activity(&self) {
        self.last_activity.store(Self::now_ms(), Ordering::Relaxed);
    }

    /// Hold off idle model unload until the guard is dropped (import / long-running ASR).
    pub fn acquire_keepalive(&self, reason: &'static str) -> KeepAliveGuard {
        self.active_leases.fetch_add(1, Ordering::SeqCst);
        debug!("TranscriptionManager keepalive acquired ({})", reason);
        self.touch_activity();
        KeepAliveGuard {
            ...
            && self.is_model_loaded()
        {
            #[cfg(target_os = "windows")]
            {
                ...
                thread::spawn(move || {
                    ...
                    info!(
                        "Immediately unloading model after {} (deferred on Windows)",
                        context
                    );
    ...
    pub fn load_model(&self, model_id: &str) -> Result<()> {
        ...
        let engine_type = model_info
            .engine_type
            .clone()
            .ok_or_else(|| anyhow::anyhow!("Model {} is not a transcription model", model_id))?;
    ...
    pub fn transcribe(&self, audio: Vec<f32>) -> Result<String> {
        ...
        let is_whisper = self
            .model_manager
            .get_model_info(&settings.selected_model)
            .map(|info| matches!(info.engine_type, Some(EngineType::Whisper)))
            .unwrap_or(false);
...
pub fn apply_accelerator_settings(app: &tauri::AppHandle) {
    ...
    let whisper_gpu_available = accel::WhisperAccelerator::available()
        .contains(&accel::WhisperAccelerator::Gpu);

    let whisper_pref = match settings.whisper_accelerator {
        WhisperAcceleratorSetting::Auto if whisper_gpu_available => accel::WhisperAccelerator::Auto,
        WhisperAcceleratorSetting::Auto => {
            info!("Whisper auto accelerator requested, but this build has no Whisper GPU backend; using CPU");
            accel::WhisperAccelerator::CpuOnly
        }
        WhisperAcceleratorSetting::Cpu => accel::WhisperAccelerator::CpuOnly,
        WhisperAcceleratorSetting::Gpu if whisper_gpu_available => accel::WhisperAccelerator::Gpu,
        WhisperAcceleratorSetting::Gpu => {
            warn!("Whisper GPU requested, but this build has no Whisper GPU backend");
            accel::WhisperAccelerator::CpuOnly
        }
    };
...
pub fn get_available_accelerators() -> AvailableAccelerators {
    ...
    let whisper_options: Vec<String> = WhisperAccelerator::available()
        .into_iter()
        .map(|a| a.to_string())
        .collect();
