...
impl WorkspaceManager {
    ...
    pub(crate) fn migrations() -> Migrations<'static> {
        Migrations::new(vec![
            ...
            "#),
            // Buddy System B1 — five-table cluster. Folded in here (rather than
            // shipped via BuddyManager::migrations()) because rusqlite_migration
            // shares one PRAGMA user_version per connection. Two managers each
            // calling .to_latest() on the same conn would panic the second one
            // with DatabaseTooFarAhead. The single ordered chain keeps version
            // accounting honest. BuddyManager::migrations() is still exposed
            // for in-memory test setup where the buddy chain runs in isolation.
            M::up(
                r#"
