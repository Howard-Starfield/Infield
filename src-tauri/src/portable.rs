...
use crate::app_identity::{LEGACY_PORTABLE_MAGIC, PORTABLE_MAGIC};

/// Portable mode support for Infield.
///
...
pub fn init() {
    PORTABLE_DATA_DIR.get_or_init(|| {
        ...
        if is_portable {
            ...
            // #region agent log
            crate::debug_session_log::write_line(
                "H1",
                "portable.rs:init",
                "portable_mode_active",
                serde_json::json!({
...
fn is_valid_portable_marker(path: &std::path::Path) -> bool {
    std::fs::read_to_string(path)
        .map(|s| {
            let trimmed = s.trim();
            trimmed.starts_with(PORTABLE_MAGIC) || trimmed.starts_with(LEGACY_PORTABLE_MAGIC)
        })
        .unwrap_or(false)
}
...
mod tests {
    ...
    fn test_valid_magic_string_enables_portable() {
        let dir = std::env::temp_dir().join("infield_test_valid");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        let mut f = std::fs::File::create(&marker).unwrap();
        write!(f, "Infield Portable Mode").unwrap();
    ...
    fn test_empty_file_does_not_enable_portable() {
        let dir = std::env::temp_dir().join("infield_test_empty");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
    ...
    fn test_wrong_content_does_not_enable_portable() {
        let dir = std::env::temp_dir().join("infield_test_wrong");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        let mut f = std::fs::File::create(&marker).unwrap();
    ...
    fn test_legacy_empty_marker_without_data_dir_does_not_enable_portable() {
        ...
        let dir = std::env::temp_dir().join("infield_test_legacy_no_data");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        std::fs::File::create(&marker).unwrap();
        assert!(!is_valid_portable_marker(&marker));
    ...
    fn test_magic_string_with_whitespace_enables_portable() {
        let dir = std::env::temp_dir().join("infield_test_ws");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        let mut f = std::fs::File::create(&marker).unwrap();
        write!(f, "  Infield Portable Mode\n").unwrap();
        assert!(is_valid_portable_marker(&marker));
        std::fs::remove_dir_all(dir).unwrap();
    }

    #[test]
    fn test_legacy_handy_marker_still_recognized_for_migration() {
        // Existing portable installs created by older Handy releases keep working
        // until the LEGACY_PORTABLE_MAGIC constant is retired (see migration.md).
        let dir = std::env::temp_dir().join("infield_test_legacy_magic");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        let mut f = std::fs::File::create(&marker).unwrap();
