...
fn paste_via_clipboard(
    ...
    info!("paste_via_clipboard: reading clipboard");
    let clipboard = app_handle.clipboard();
    let clipboard_content = clipboard.read_text().unwrap_or_default();
    info!("paste_via_clipboard: writing text to clipboard");
    ...
    write_result?;
    info!(
        "paste_via_clipboard: clipboard written, sleeping {}ms",
        paste_delay_ms
    );
    std::thread::sleep(Duration::from_millis(paste_delay_ms));
    info!("paste_via_clipboard: sleep done, sending key combo");

    // #region agent log
    #[cfg(target_os = "windows")]
    crate::debug_session_log::write_line(
        "H-win-paste",
        "clipboard.rs:paste_via_clipboard",
        "before_key_combo",
        serde_json::json!({ "paste_method": format!("{:?}", paste_method) }),
    );
    // #endregion
...
fn paste_direct(
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // `enigo.text()` can fault on Windows (native crash). Use the same clipboard + SendInput
        // path as Ctrl+V (see `input::send_paste_ctrl_v`).
        return paste_via_clipboard(enigo, text, app_handle, &PasteMethod::CtrlV, paste_delay_ms);
    }
...
fn send_return_key(enigo: &mut Enigo, key_type: AutoSubmitKey) -> Result<(), String> {
    ...
    {
        let _ = enigo;
        return input::windows_send_auto_submit_return(key_type);
    }
...
pub fn paste(text: String, app_handle: AppHandle) -> Result<(), String> {
    ...
    info!("paste: acquiring Enigo lock");
    ...
    info!("paste: Enigo lock acquired");

    // Perform the paste operation
    match paste_method {
        PasteMethod::None => {
            unreachable!("PasteMethod::None returns before Enigo is acquired");
        }
        PasteMethod::Direct => {
            paste_direct(
                &mut enigo,
