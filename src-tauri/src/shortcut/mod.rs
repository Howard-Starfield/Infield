...
#[tauri::command]
#[specta::specta]
pub fn change_show_onboarding_setting(app: AppHandle, enabled: bool) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.show_onboarding = enabled;
    settings::write_settings(&app, settings);

    let _ = app.emit(
        "settings-changed",
        serde_json::json!({
            "setting": "show_onboarding",
            "value": enabled
        }),
    );
...
#[tauri::command]
#[specta::specta]
pub fn change_system_audio_paragraph_silence_secs_setting(
    app: AppHandle,
    secs: f32,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
...
#[tauri::command]
#[specta::specta]
pub fn change_system_audio_vad_hangover_secs_setting(
    app: AppHandle,
    secs: f32,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
...
#[tauri::command]
#[specta::specta]
pub fn change_auto_create_note_setting(app: AppHandle, enabled: bool) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.auto_create_note = enabled;
    settings::write_settings(&app, settings);
    if !enabled {
        if let Some(voice_session_manager) = app.try_state::<std::sync::Arc<VoiceSessionManager>>()
        {
...
#[tauri::command]
#[specta::specta]
pub fn change_auto_tag_enabled_setting(app: AppHandle, enabled: bool) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.auto_tag_enabled = enabled;
    settings::write_settings(&app, settings);
    let _ = app.emit(
        "settings-changed",
        serde_json::json!({ "setting": "auto_tag_enabled", "value": enabled }),
    );
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn change_llm_model_path_setting(app: AppHandle, path: Option<String>) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.llm_model_path = path.filter(|value| !value.trim().is_empty());
    settings::write_settings(&app, settings);
    if let Some(llm_manager) = app.try_state::<std::sync::Arc<LlmManager>>() {
        llm_manager
...
#[tauri::command]
#[specta::specta]
pub fn change_llm_gpu_enabled_setting(app: AppHandle, enabled: bool) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.llm_gpu_enabled = enabled;
    settings::write_settings(&app, settings);
    if let Some(llm_manager) = app.try_state::<std::sync::Arc<LlmManager>>() {
        llm_manager
            .reload_from_settings(&app)
