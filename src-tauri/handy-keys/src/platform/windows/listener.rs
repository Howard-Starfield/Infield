...
unsafe extern "system" fn keyboard_hook_proc(code: i32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    ...
    HOOK_CONTEXT.with(|ctx_cell| {
        ...
        if let Some(ctx) = ctx_ref.as_mut() {
            ...
            // Skip synthetic (injected) keystrokes — e.g. from enigo's SendInput.
            // Processing injected events can corrupt modifier state and cause crashes
            // when the hook tries to block/unblock keys that were never physically pressed.
